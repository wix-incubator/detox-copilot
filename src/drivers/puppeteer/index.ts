...
export class TestingFrameworkDriverImpl implements TestingFrameworkDriver {
  ...
  async captureSnapshotImage(): Promise<string | undefined> {
    if (!this.currentPage) {
      throw new Error("No active page to capture snapshot from");
    }
    ...
    await this.currentPage.screenshot({ path: fileName });
    return fileName;
  }
  ...
  async captureViewHierarchyString(): Promise<string> {
    if (!this.currentPage) {
      return "CANNOT SEE ANY ACTIVE PAGE, START A NEW ONE BASED ON THE ACTION NEED OR RAISE AN ERROR";
    }
    // todo: we'll need to add puppeteer as dev dependency (for types) and as a peer dependency (for the actual package).
    // @ts-expect-error this is a puppeteer method
